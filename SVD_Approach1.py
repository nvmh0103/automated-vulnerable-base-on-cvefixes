...
def toPickle(dataSetType):
    global args
    # List all groups
    data = h5py.File("dataset/"+args['dataset'][0]+"_"+dataSetType+".hdf5",'r')
    data.visit(print)
    ...
    mydf.iloc[0:5,0:]
    mydf.to_pickle("pickle_file/"+args['dataset'][0]+"_"+dataSetType+".pickle")
...
def main(): 
    ...
    global logger
    logger = svdc.configureLogging(args['loglevel'], args['console'])
    if(args['pickle']):
        convert2Pickle()
    if(args['train']):
        trainModel()
